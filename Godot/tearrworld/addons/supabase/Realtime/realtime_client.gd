...
func connect_client() -> int:
	...
	var url: String = "{url}?apikey={apikey}".format({url = _db_url, apikey = _apikey})
	var auth_client = Supabase.auth.client
	if auth_client and auth_client.session and auth_client.session.access_token != "":
		var token: String = auth_client.session.access_token
		
		url += "&access_token=" + token
		
		var headers_temp := Array(handshake_headers)
		headers_temp.append("Authorization: Bearer " + token)
		handshake_headers = PackedStringArray(headers_temp)
		print("🔑 [EXTENSION REALTIME] Session utilisateur détectée ! Injection automatique du jeton JWT.")
	else:
		print("📡 [EXTENSION REALTIME] Connexion en mode anonyme (clé API standard uniquement).")
	_ws_client.handshake_headers = handshake_headers
	
	var err := _ws_client.connect_to_url(url, tls_options)
	if err != OK:
		return err
